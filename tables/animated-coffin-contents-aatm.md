---
obsidianUIMode: preview
cssclasses:
- json5e-note
tags:
- compendium/src/5e/aatm
aliases:
- "Animated Coffin Contents"
---
# Animated Coffin Contents
*Source: Adventure Atlas: The Mortuary* 

**Animated Coffin Contents**

| dice: d6 | Contents |
|----------|----------|
| 1 | `1d4` [swarm of bats](/bestiary/beast/swarm-of-bats-xmm.md) |
| 2 | `1d4` [skeletons](/bestiary/undead/skeleton-xmm.md) packed like sardines |
| 3 | A groaning [mummy](/bestiary/undead/mummy-xmm.md) |
| 4 | A patch of [yellow mold](/traps-hazards/yellow-mold-xdmg.md) (see the "Dungeon Master's Guide") |
| 5 | A slumbering [vampire spawn](/bestiary/undead/vampire-spawn-xmm.md) |
| 6 | A portal to the Path of Graves (detailed earlier in this supplement) |
^animated-coffin-contents